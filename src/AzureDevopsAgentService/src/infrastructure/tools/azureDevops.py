...
DUMMY_WORK_ITEMS_DB = {
    201: {
        "id": 201,
        "type": "User Story",
        "title": "User Profile Page Refactor",
        "status": "Active",
    ...
    205: {
        ...
        "tags": ["bug", "export", "backend", "performance"],
    ...
    206: {
        "id": 206,
        "type": "Task",
        "title": "Configure Staging Environment Monitoring",
        "status": "Closed",
        "assigned_to": "Eve Davis",
        "created_date": "2025-03-15",
        "tags": ["devops", "monitoring", "staging"],
    ...
    207: {
        "id": 207,
        "type": "User Story",
        "title": "Allow users to customize dashboard widgets",
        "status": "New",
        "assigned_to": "Alice Johnson",
        "created_date": "2025-03-22",
        "tags": ["frontend", "dashboard", "feature"],
    ...
    209: {
        "id": 209,
        "type": "Task",
        "title": "Perform Security Audit on Dependencies",
        "status": "Active",
        "assigned_to": "Eve Davis",
    ...
    210: {
        ...
        "tags": ["integration", "notifications", "feature", "backend"],
...
class ListWorkItemsArgs(BaseModel):
    ...
    start_date: Optional[str] = Field(
        None,
        description="Filter work items created or updated after this date (YYYY-MM-DD).",
    )
    end_date: Optional[str] = Field(
        None,
        description="Filter work items created or updated before this date (YYYY-MM-DD).",
    )
    item_type: Optional[str] = Field(
        None,
        description="Filter by work item type (e.g., 'Bug', 'User Story', 'Task').",
    )
    status: Optional[str] = Field(
        None,
        description="Filter by work item status (e.g., 'New', 'Active', 'Resolved', 'Closed').",
    )
    tags: Optional[List[str]] = Field(
        None, description="Filter by tags associated with the work item."
    )
    assigned_to: Optional[str] = Field(
        None,
        description="Filter by the person the work item is assigned to (email or name).",
    )
...
async def listWorkItems(
    ...
    """
    print(
        f"Tool 'listWorkItems' called with filters: start_date={start_date}, end_date={end_date}, item_type={item_type}, status={status}, tags={tags}, assigned_to={assigned_to}"
    )
    ...
    if start_date:
        try:
            ...
            filtered_items = [
                ...
                if datetime.datetime.strptime(item.created_date, "%Y-%m-%d").date()
                >= start_dt
            ]
        except ValueError:
            print(
                f"Warning: Invalid start_date format: {start_date}. Should be YYYY-MM-DD."
            )
    if end_date:
        try:
            ...
            print(
                f"Warning: Invalid end_date format: {end_date}. Should be YYYY-MM-DD."
            )
    if item_type:
        filtered_items = [
            item for item in filtered_items if item.type.lower() == item_type.lower()
        ]
    if status:
        filtered_items = [
            item for item in filtered_items if item.status.lower() == status.lower()
        ]
    if assigned_to:
        filtered_items = [
            item
            for item in filtered_items
            if assigned_to.lower() in item.assigned_to.lower()
        ]
    if tags:
        ...
        filtered_items = [
            item
            for item in filtered_items
            if any(tag.lower() in lower_tags for tag in item.tags)
        ]

    # --- Add check for empty results ---
    if not filtered_items:
        print("No matching work items found. Returning message.")
...
class GetWorkItemByIdArgs(BaseModel):
    ...
    work_item_id: int = Field(
        ..., description="The unique ID of the Azure DevOps work item to retrieve."
    )
...
async def getWorkItemById(work_item_id: int) -> WorkItem:
    ...
    if item_data:
        ...
        return WorkItem(
            id=work_item_id,
            type="Unknown",
            title=f"Work Item {work_item_id} Not Found",
            status="NotFound",
            assigned_to="N/A",
            created_date="N/A",
            description="This work item was not found in the dummy data.",
            tags=[],
